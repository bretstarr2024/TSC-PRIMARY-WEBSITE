# Tech Stack: The Starr Conspiracy Smart Website

**Last Updated:** February 25, 2026 | **Version:** 0.28.0
...
npm run build
  ...
  3. index-content →  Generate OpenAI embeddings for RAG (skips without MONGODB_URI)
```
...
npm run generate-content
  → Reads tsc.json + content-prompts.ts
  → Calls OpenAI gpt-4o via pure fetch
  → Stores in MongoDB tsc.* collections
  → Multi-cluster weighted round-robin across 3 JTBD clusters
  → Expert attribution rotation
```
